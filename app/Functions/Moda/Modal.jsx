              ...
              }} dragConstraints={{ top: 0, bottom: 0 }} dragElastic={.5} drag={isDrag ? null : 'y'} className={`z-[100000000000] ${className}`} layoutId={select}>
                {
                  isDrag ?
                    <motion.i title={` click to close or use the ESC key to close modal.`} onClick={e => {
                      setselect(null);
                    }} className="bi fixed top-3 right-5 bi-x-lg h-10 w-10 min-w-10 min-h-10 flex items-center justify-center brd rounded-xl shadow-md  z-[100000000000000] bg-[var(--baseBg)] hover:bg-[#c73a3a] hover:text-gray-500 text-gray-500 cursor-pointer" /> : ''
                }
                {children}
              </motion.div>
            </motion.div>
